...
export class NotificationsService {
  ...
  protected convertDateFromClient(notifications: INotifications): INotifications {
    const copy: INotifications = Object.assign({}, notifications, {
      times: notifications.times && notifications.times.isValid() ? notifications.times.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((notifications: INotifications) => {
        notifications.times = notifications.times ? moment(notifications.times) : undefined;
      });
    }
    return res;
  }
